...
double external_potential(double val)
{
    ...
    return val*val*val*val-5.0*val*val/2.0;
}
...
double external_potential_prime(double val)
{
    ...
    return 4.0*val*val*val-5.0*val;
}
...
double variationalWaveFunction(double v)
{
	//return 1.0;
	//return exp(-0.5*v*v);
    double sig=0.6071428571428571, mu=0.8061224489795918;
    return exp( -((v-mu)*(v-mu))/(2.0*sig*sig) ) + exp( -((v+mu)*(v+mu))/(2.0*sig*sig) );
}
...
double variationalWaveFunction_second(double v)
{
    ...
    double sig=0.6071428571428571, mu=0.8061224489795918;
    return exp( -((v+mu)*(v+mu))/(2.0*sig*sig) ) * ( -sig*sig + exp( (2.0*mu*v)/(sig*sig) )*(-mu-sig+v)*(-mu+sig+v) +(mu+v)*(mu+v)  ) / pow(sig,4.0);
}
