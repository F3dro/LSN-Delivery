...
void Averages(int iblk) //Print results for current block
{
   ...
   if(RealUnits==true){
     Epot.open("output.epot_" + to_string(material) + ".dat",ios::app);
     Pres.open("output.pres_" + to_string(material) + ".dat",ios::app);
     Gofr.open("output.gofr_" + to_string(material) + ".dat",ios::app);
...
//Pressure
    ...
    for(int i=igofr; i<n_props; i++){   //g(r)
      ...
      stima_gofr = blk_av[i]/blk_norm/(rho*npart*4./3.*M_PI*d_vol);
